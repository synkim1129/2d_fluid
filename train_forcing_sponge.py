...
tau = torch.zeros( 2, h, w)
layerd=5
cond_x=5
cond_y=3
gamma=0.2
for i in range(layerd):
    tau[:,cond_y+i,:]=gamma*np.sin(np.pi/2*(1-(i/layerd)))**2
    #tau[:,:,cond_x+i]=gamma*np.sin(np.pi/2*(1-(i/layerd)))**2
    tau[:,h-cond_y-i,:]=gamma*np.sin(np.pi/2*(1-(i/layerd)))**2
    #tau[:,:,w-cond_x-i]=gamma*np.sin(np.pi/2*(1-(i/layerd)))**2
tau = tau.cuda()
...
for epoch in range(params.load_index, params.n_epochs):
	...
	for i in range(params.n_batches_per_epoch):
		...
		loss_nav =  torch.mean(loss_function(flow_mask_mac[:,1:2]*(rho*((v_new[:,1:2]-v_old[:,1:2])/dt+v[:,1:2]*dx(v[:,1:2])+0.5*(map_vy2vx_top(v[:,0:1])*dy_top(v[:,1:2])+map_vy2vx_bottom(v[:,0:1])*dy_bottom(v[:,1:2])))+dx_left(p_new)-mu*laplace(v[:,1:2])+damper[:,1:2]))[:,:,1:-1,1:-1],dim=(1,2,3))+\
                torch.mean(loss_function(flow_mask_mac[:,0:1]*(rho*((v_new[:,0:1]-v_old[:,0:1])/dt+v[:,0:1]*dy(v[:,0:1])+0.5*(map_vx2vy_left(v[:,1:2])*dx_left(v[:,0:1])+map_vx2vy_right(v[:,1:2])*dx_right(v[:,0:1])))+dy_top(p_new)-mu*laplace(v[:,0:1])+damper[:,0:1]))[:,:,1:-1,1:-1],dim=(1,2,3))
