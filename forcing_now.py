...
vobs_file='/home/teamai/data/diff1000ep240/just/u_v_shifted_time.nc'
...
for i in range(n_forcing):
    v_obs[:, 1, y_obs[i], x_obs[i]] =torch.tensor(u_obs_adj[:, y_obs[i]-3-spongebuf, x_obs[i]-5], dtype=torch.float32)
    v_obs[:, 0, y_obs[i], x_obs[i]] =torch.tensor(v_obs_adj[:, y_obs[i]-3-spongebuf, x_obs[i]-5], dtype=torch.float32)
...
nc_file_v_new = '/home/teamai/data/diff1000ep240/shift_2.5*np.pi_4/v_new_data.nc'
