...
import torch
import numpy as np
from setups_demo_mask import Dataset
...
#x_obs = [200,200,195,195,190,190,185,185,180,180]
#x_obs = [190,190,185,185,180,180,175,175,170,170]
x_obs = [180,175,175,170,170,170,165,165,165,165]
#x_obs = [195,195,175,175,155,155,135,135,115,115]
#x_obs = [850,850,825,825,800,800,775,775,750,750]
y_obs = [50,55,45,57,50,43,60,55,45,40]
#y_obs = [45,55,45,55,45,55,45,55,45,55]
#y_obs = [25,70,25,70,25,70,25,70,25,70]
...
obs_mask = torch.zeros(v_obs_timesize, 1, h, w)
spongebuf=25
print(n_forcing)
...
with torch.no_grad():
    while True:
        ...
        dataset = Dataset(w,h,1,1,interactive=True,average_sequence_length=n_time_steps,max_speed=params.max_speed,dt=dt,types=["fixed_ball"],images=["fish"],background_images=["empty"], forcing=True)
        ...
        for t in range(n_time_steps):
            v_cond,cond_mask,flow_mask,a_old,p_old, _, _, _, _ = toCuda(dataset.ask())
            ...
            a_new,p_new = fluid_model(a_old,p_old,flow_mask,v_cond,cond_mask,v_obs[t][np.newaxis,:],obs_mask[t][np.newaxis,:])
            v_new = rot_mac(a_new)
