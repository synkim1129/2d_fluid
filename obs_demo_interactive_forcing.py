...
import torch
import numpy as np
from obs_setups_demo import Dataset
...
date_time,index = logger.load_state(fluid_model,None,direct_path='./obs_state/1p/500.state')
fluid_model.eval()
...
if save_movie:
    ...
    movie_p = cv2.VideoWriter(f'./obs_data/peak/p_forcing_before; {get_param.get_hyperparam(params)}.avi', fourcc, 20.0, (w,  h))
    movie_v = cv2.VideoWriter(f'./obs_data/peak/v_forcing_before; {get_param.get_hyperparam(params)}.avi', fourcc, 20.0, (w-3,  h-3))
    movie_a = cv2.VideoWriter(f'./obs_data/peak/a_forcing_before; {get_param.get_hyperparam(params)}.avi', fourcc, 20.0, (w,  h))
...
vobs_file='./obs_data/before/v_new_data.nc'
with nc.Dataset(vobs_file, 'r') as ds:
    # v_obs_adj = ds.variables['v_amplified'][:4100]
    # u_obs_adj = ds.variables['u_amplified'][:4100]
    v_obs_adj = ds.variables['v_new'][:]
    u_obs_adj = ds.variables['u_new'][:]
...
spongebuf=25
v_obs[:, 0, y_obs, x_obs] =torch.tensor(u_obs_adj[:, y_obs-3-spongebuf, x_obs-5], dtype=torch.float32)
v_obs[:, 1, y_obs, x_obs] =torch.tensor(v_obs_adj[:, y_obs-3-spongebuf, x_obs-5], dtype=torch.float32)
v_obs = v_obs.cuda()
x_indces[0]=x_obs
y_indces[0]=y_obs


nc_file_v_new = './obs_data/peak/v_new_data_before.nc'
