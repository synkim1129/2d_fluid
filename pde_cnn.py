...
class PDE_UNet2(nn.Module):
	...
	def __init__(self, hidden_size=64,bilinear=True):
		...
		self.inc = DoubleConv(12, hidden_size)
		self.down1 = Down(hidden_size, 2*hidden_size)
		self.down2 = Down(2*hidden_size, 4*hidden_size)
		self.down3 = Down(4*hidden_size, 8*hidden_size)
		factor = 2 if bilinear else 1
	...
	def forward(self,v_old,H_old,mask_flow,v_cond,mask_cond):
		x = torch.cat([v_old, H_old, mask_flow, v_cond*mask_cond, mask_cond, mask_flow*v_old, mask_flow*H_old, v_old*mask_cond],dim=1)
		x1 = self.inc(x)
		x2 = self.down1(x1)
		x3 = self.down2(x2)
		x4 = self.down3(x3)
		...
		x = self.up4(x, x1)
		x = self.outc(x)
		v_new, H_new = v_old+x[:,0:2], H_old+x[:,2:3]
		return v_new,H_new
