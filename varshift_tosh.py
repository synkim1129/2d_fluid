...
# 입력 및 출력 파일 경로 설정
inputdir = '/home/teamai/data/expname/just/'
outputdir = inputdir
...
spinup = 1000
forplot = 1700
phase_shift_value1 = shiftphase1  # 위상 shift 값 설정
phase_shift_value2 = shiftphase2  # 위상 shift 값 설정
target_freq = targetfreq # 목표로 하는 특정 wavenumber
...
def apply_phase_shift(data, phase_shift_value1,phase_shift_value2, target_freq, bandwidth):
    # Fourier 변환 (time 방향)
    ...
    # 위상 shift 적용
    phase_shift1 = np.ones_like(data_hat, dtype=complex)  # 기본적으로 shift 없음
    phase_shift2 = np.ones_like(data_hat, dtype=complex)  # 기본적으로 shift 없음
    for idx in band_indices:
        phase_shift1[idx, :, :] = np.exp(1j * phase_shift_value1)  # 특정 주파수 대역에만 위상 shift 적용
        phase_shift2[idx, :, :] = np.exp(1j * phase_shift_value2)  # 특정 주파수 대역에만 위상 shift 적용
    # 주파수 도메인에서 위상 shift 적용
    data_hat_shifted1 = data_hat * phase_shift1
    data_hat_shifted2 = data_hat * phase_shift2

    # 역 Fourier 변환
    data_shifted1 = np.fft.ifft(data_hat_shifted1, axis=0)
    data_shifted2 = np.fft.ifft(data_hat_shifted2, axis=0)
    data_shifted1 = np.real(data_shifted1)  # 실수부만 사용
    data_shifted2 = np.real(data_shifted2)  # 실수부만 사용
    data_shifted=data_shifted1
    data_shifted[int(data_shifted.shape[0]/2):]=data_shifted2[int(data_shifted.shape[0]/2):]

    return data_shifted, data_hat, data_hat_shifted1
...
u_shifted, u_hat, u_hat_shifted = apply_phase_shift(u_new, phase_shift_value1, phase_shift_value2, target_freq,bandwidth)

# v_new에 대해 shift 적용 및 에너지 계산
v_shifted, v_hat, v_hat_shifted = apply_phase_shift(v_new, phase_shift_value1, phase_shift_value2, target_freq,bandwidth)
