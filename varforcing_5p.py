...
if save_movie:
    ...
    movie_p = cv2.VideoWriter(f'plots/p_forcing_{get_param.get_hyperparam(params)}.avi', fourcc, 20.0, (w,  h))
    movie_v = cv2.VideoWriter(f'plots/v_forcing_{get_param.get_hyperparam(params)}.avi', fourcc, 20.0, (w-3,  h-3))
    movie_a = cv2.VideoWriter(f'plots/a_forcing_{get_param.get_hyperparam(params)}.avi', fourcc, 20.0, (w,  h))
...
#vobs_file='/home/teamai/data/long/just/u_v_shifted.nc'
vobs_file='/home/teamai/data/expname/just/u_v_shifted_time.nc'
#vobs_file='/home/teamai/data/241114/just/u_v_shifted_time.nc'
with nc.Dataset(vobs_file, 'r') as ds:
    v_obs_adj = ds.variables['v_shifted'][:]
    u_obs_adj = ds.variables['u_shifted'][:]
...
#x_obs = [200,200,190,190,180,180,170,170,160,160]
#x_obs = [204,204,202,202,200,200,198,198,196,196]
#x_obs = [210,210,208,208,205,205,200,200,195,195]
#x_obs = [212,212,207,207,202,202,197,197,192,192] 
x_obs = [213,213,208,208,195,195,190,190,185,185] #best
#x_obs = [213,213,205,205,195,195,190,190,185,185]
#x_obs = [210,210,207,207,204,204,201,201,198,198]
#x_obs = [200,200,190,190,185,185,180,180,175,175]
#x_obs = [200,195,195,190,190,190,185,185,185,185]
#x_obs = [195,195,175,175,155,155,135,135,115,115]
#x_obs = [850,850,825,825,800,800,775,775,750,750]
#y_obs = [60,50,40,60,50,40,65,57,43,35]
#y_obs = [40,60,40,60,40,60,40,60,40,60]
y_obs = [45,55,40,60,35,65,35,65,35,65] #best
...
#x_obs = 200
#y_obs = 50
v_obs = torch.zeros(v_obs_timesize, 2, h, w)
spongebuf=25
print(n_forcing)
for i in range(n_forcing):
    v_obs[:, 0, y_obs[i], x_obs[i]] =torch.tensor(u_obs_adj[:, y_obs[i]-3-spongebuf, x_obs[i]-5], dtype=torch.float32)
    v_obs[:, 1, y_obs[i], x_obs[i]] =torch.tensor(v_obs_adj[:, y_obs[i]-3-spongebuf, x_obs[i]-5], dtype=torch.float32)
...
#nc_file_v_new = '/home/teamai/data/241014/just/v_new_data.nc'
#nc_file_v_new = '/home/teamai/data/long/shift/v_new_data.nc'
nc_file_v_new = '/home/teamai/data/expname/var_shiftphase/v_new_data.nc'
#nc_file_v_new = '/home/teamai/data/241114/shift/v_new_data.nc'
...
def save_to_netcdf(v_new, flow_mask, t):
    with nc.Dataset(nc_file_v_new, 'a') as ds_v_new:
        ...
        u_var[current_time_len, :, :] = v_new.cpu().numpy()[0,0,:,:]
        v_var[current_time_len, :, :] = v_new.cpu().numpy()[0,1,:,:]
...
with torch.no_grad():
    while True:
        ...
        for t in range(n_time_steps):
            ...
            a_new,p_new = fluid_model(a_old,p_old,flow_mask,v_cond,cond_mask,v_obs[t][np.newaxis,:])
            v_new = rot_mac(a_new)
            


            save_to_netcdf(v_new,flow_mask,t)
