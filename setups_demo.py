...
class Dataset:
	...
	def reset_env(self,index):
		...
		if type == "fixed_ball": # magnus effekt (1)
			flow_v = -self.max_speed #flow velocity (1.5) (before: 3*(np.random.rand()-0.5)*2)
			object_y = self.h//2
			object_x = 3*self.w//4
			object_r = 10 # object radius (15)
			object_vx = self.init_velocity*(np.random.rand()-0.5)*2
			object_vy = self.init_velocity*(np.random.rand()-0.5)*2
			object_w = 0 # object angular velocity (3/object_r)
		...
		if type == "empty":# block at random position
			flow_v = -self.max_speed
		...
		if type == "fixed_box":# block at random position
			object_h = 10# object height / 2
			object_w = 10 # object width / 2
			flow_v = -self.max_speed
			object_y = self.h//2
			object_x = 3*self.w//4
			object_vx = self.init_velocity*(np.random.rand()-0.5)*2
